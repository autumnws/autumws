...
select emp.full_name, sd.name, od.name,a.created_date  from allocations a
inner join space_data sd on sd.id = a.space_id 
inner join employee emp on emp.id = a.employee_id
inner join org_data od on od.id = a.project_id 
inner join request_data rd on rd.id = a.request_id 
where a.type = 'SEAT' and a.is_active = 1;
